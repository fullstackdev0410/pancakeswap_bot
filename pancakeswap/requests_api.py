...
class PancakeSwapAPI:
    ...
    def tokens(self, address: str = None):
        ...
        Otherwise, returns the swap_methods in the top ~1000 pairs on PancakeSwap, sorted by reserves.
        :type address: str
        :return: Dict
        """
        if address:
        ...
        url = f"{self.base_url}swap_methods{'/' + address if address is not None else ''}"
        return self.__get(url)
